...
test.describe('Backtest App E2E Tests', () => {
  ...
  test('should load the main page and display key components', async ({ page }) => {
  ...
  test('should handle strategy input and validation', async ({ page }) => {
    ...
    await strategyInput.fill('5日移動平均が20日移動平均を上回ったら買い、下回ったら売り');
  ...
  test('should display progress during backtest execution', async ({ page }) => {
  ...
  test('should render results chart within 5 seconds', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="equity-curve-chart"]')).toBeVisible({ timeout: 5000 });
  ...
  test('should display backtest metrics correctly', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="metrics-display"]')).toBeVisible({ timeout: 10000 });
  ...
  test('should display trades table with correct format', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="trades-table"]')).toBeVisible({ timeout: 10000 });
    
    // Check table headers
    const table = page.locator('[data-testid="trades-table"]');
    await expect(table.locator('th')).toContainText(['銘柄', 'エントリー日', 'エグジット日', '数量', 'P&L']);
  ...
  test('should handle stock period selection', async ({ page }) => {
    ...
    if (await periodSelector.isVisible()) {
      ...
      // Select a period
      const periodOption = page.locator('[data-testid="period-option"]').first();
